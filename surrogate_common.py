...
def write_mdif(
    ...
    *,
    ...
    lines = list(
        header_lines
        or [
            "! Predicted S-parameter MDIF",
            f"! Generated by ads-ann-surrogate {VERSION}",
        ]
    )
    lines.append("")
    for block in blocks:
...
def write_ads_export_template(
    path: Path,
    blocks: Sequence[MDIFBlock],
    parameter_names: Sequence[str],
    sparam_labels: Sequence[str],
) -> dict[str, object]:
    """Write the fitted data grid as a reusable sampled-MDIF export template.

    The exporter uses only the template's model parameters and frequencies. Its
    S-parameter values are deliberately zero placeholders so the file cannot be
    mistaken for either measured data or a surrogate prediction.
    """

    if not blocks:
        raise ValueError("Cannot create an ADS export template without MDIF blocks")
    if not parameter_names:
        raise ValueError("Cannot create an ADS export template without model parameters")
    if not sparam_labels:
        raise ValueError("Cannot create an ADS export template without S-parameters")

    template_blocks: list[MDIFBlock] = []
    for template_index, block in enumerate(blocks, start=1):
        missing = [name for name in parameter_names if name not in block.params]
        if missing:
            raise ValueError(
                f"Block {block.source_index} is missing export-template parameter(s): "
                + ", ".join(missing)
            )
        frequencies = np.asarray(block.freq_hz, dtype=float).copy()
        template_blocks.append(
            MDIFBlock(
                params={name: block.params[name] for name in parameter_names},
                freq_hz=frequencies,
                sparams={
                    label: np.zeros(frequencies.shape, dtype=complex)
                    for label in sparam_labels
                },
    ...
    write_mdif(
        ...
        sparam_labels,
        header_lines=[
            "! ADS sampled-export template; S-parameter values are zero placeholders",
            "! Parameter and frequency blocks were copied from the model-fitting input",
            f"! Generated by ads-ann-surrogate {VERSION}",
        ],
    )
    all_frequencies = np.concatenate(
        [np.asarray(block.freq_hz, dtype=float) for block in template_blocks]
    )
    return {
        "filename": path.name,
        "block_count": len(template_blocks),
        "frequency_row_count": int(all_frequencies.size),
        "frequency_min_hz": float(np.min(all_frequencies)),
        "frequency_max_hz": float(np.max(all_frequencies)),
        "parameter_names": list(parameter_names),
        "sparam_labels": list(sparam_labels),
        "sparameter_values": "zero_placeholders",
    }
...
def build_ads_export_blocks(
    ...
    if not template_mdif and not parameter_grid_specs and model_dir is not None:
        generated_template = Path(model_dir) / ADS_EXPORT_TEMPLATE_FILENAME
        if generated_template.is_file():
            template_mdif = str(generated_template)
    if template_mdif:
    ...
    if not parameter_grid_specs:
        generated_hint = (
            f"; no generated template was found at "
            f"{Path(model_dir) / ADS_EXPORT_TEMPLATE_FILENAME}"
            if model_dir is not None
            else ""
        )
        raise ValueError(
            "Provide --template-mdif or at least one --parameter-grid"
            + generated_hint
        )
    if not freqs_spec:
...
ADS_ANN_NETLIST_PLACEHOLDER = "__ADS_ANN_EQUATION_FRAGMENT__"


def _ads_ann_output_position(
    output_columns: Sequence[str],
    label: str,
    component: str,
) -> int:
    suffix = f"{label}_{component}"
    matches = [
        index
    ...
    if len(matches) != 1:
        raise ValueError(
            f"ADS ANN netlist export expected one output ending in {suffix!r}; "
            f"found {len(matches)}"
        )
    return matches[0]


def ads_ann_netlist_template(
    input_columns: Sequence[str],
    output_columns: Sequence[str],
    parameter_names: Sequence[str],
    sparam_labels: Sequence[str],
    ...
    subnetwork in advance and leaves one placeholder for that formula.
    """

    labels = [str(value) for value in sparam_labels]
    parameters = [str(value) for value in parameter_names]
    inputs = [str(value) for value in input_columns]
    outputs = [str(value) for value in output_columns]
    nports = infer_complete_sparameter_ports(labels)
    if not math.isfinite(float(z0)) or float(z0) <= 0.0:
        raise ValueError("ADS ANN netlist reference impedance must be positive and finite")
    if len(parameter_model_defaults) != len(parameters):
        raise ValueError("ADS ANN netlist parameter defaults do not match parameter names")

    coarse_inputs = [name for name in inputs if name.startswith("coarse_")]
    if coarse_inputs:
        raise ValueError(
            "A self-contained ADS ANN netlist cannot accept external coarse_* "
            "inputs. Re-export KBNN with --ads-ann-target fine and without "
            "--include-coarse-input."
        )
    residual_outputs = [name for name in outputs if name.startswith("delta_")]
    if residual_outputs:
        raise ValueError(
            "A self-contained ADS ANN netlist requires final fine S-parameter "
            "outputs, not residual delta outputs. Re-export KBNN with "
            "--ads-ann-target fine."
        )

    module_id = _ads_hb_identifier(module_name, "ads_ann_sdd")
    scale_map = {name: float(parameter_input_scales[name]) for name in parameters}
    param_ids, scale_ids, scales, instance_defaults, parameter_features = (
        _ads_hb_parameter_configuration(
            parameters,
            np.asarray(parameter_model_defaults, dtype=float),
            scale_map,
        )
    )
    feature_by_parameter = dict(zip(parameters, parameter_features))
    lines = [
        "; Self-contained RF-only native ADS ANN SDD subnetwork",
        "; The ANN equation fragment is generated by keysight.ads.ann at training time",
        "; Generated by ads-ann-surrogate",
        f"define {module_id} ({' '.join(f'p{index}' for index in range(1, nports + 1))})",
    ]
    parameter_tokens = [
        f"{identifier}={veriloga_float(default)}"
        for identifier, default in zip(param_ids, instance_defaults)
    ]
    parameter_tokens.extend(
        f"{scale_id}={veriloga_float(scale)}"
        for scale_id, scale in zip(scale_ids, scales)
    )
    if parameter_tokens:
        lines.append("parameters " + " ".join(parameter_tokens))
    lines.append("")
    lines.append("; ANN input mapping in ads_ann_manifest.json order")
    for index, column in enumerate(inputs, start=1):
        if column in feature_by_parameter:
            expression = feature_by_parameter[column]
        elif column == "freq_hz":
            expression = "max(abs(freq),1.0)"
        elif column == "freq_log10_hz":
            expression = "log10(max(abs(freq),1.0))"
        else:
            raise ValueError(
                f"ADS ANN netlist does not know how to generate input {column!r}"
            )
        lines.append(f"ann_in_{index}={expression}")

    lines.extend(
        [
            "",
            "; BEGIN native ADS ANN text formula",
            ADS_ANN_NETLIST_PLACEHOLDER,
            "; END native ADS ANN text formula",
            "",
            "; Reconstruct positive-frequency S, then enforce real-wave symmetry",
        ]
    )
    row_major_s: list[str] = [""] * (nports * nports)
    for label in labels:
        real_position = _ads_ann_output_position(outputs, label, "real")
        imag_position = _ads_ann_output_position(outputs, label, "imag")
        fitted_name = f"{module_id}_rf_{label.lower()}"
        active_name = f"{module_id}_{label.lower()}"
        lines.append(
            f"{fitted_name}=complex(ann_out_{real_position},ann_out_{imag_position})"
        )
        lines.append(
            f"{active_name}=if (freq < 0) then conj({fitted_name}) "
            f"else {fitted_name} endif"
        )
        indices = sparam_indices(label)
        if indices is None:
            raise ValueError(f"Invalid S-parameter label {label!r}")
        row, column = indices
        row_major_s[(row - 1) * nports + (column - 1)] = active_name
    if any(not name for name in row_major_s):
        raise ValueError("ADS ANN netlist requires a complete N-port S-parameter matrix")

    lines.append("")
    y_names = _append_ads_hb_s_to_y_equations(
        lines,
        f"{module_id}_stoy",
        row_major_s,
        nports,
    ...
    contract: dict[str, object] = {
        ...
        "nports": nports,
        "netlist_file": f"{module_id}.net",
        "netlist_template_file": f"{module_id}.net.in",
        "instance_template_file": "ADS_ANN_INSTANCE_TEMPLATE.txt",
...
def write_sdd_netlist(manifest, mapped_equation):
    contract = manifest.get("ads_netlist") or {}
    if not contract.get("enabled"):
        return None
    if mapped_equation is None:
        raise FileNotFoundError(
            "The ADS ANN NetlistInclude wrapper requires the native .equation "
            "artifact. Re-export with --ads-output-format all or equation, then "
            "rerun train_ads_ann.py."
        )
    equation_path = PACKAGE_DIR / mapped_equation
    equation = equation_path.read_text().strip()
    if not equation:
        raise ValueError(f"Native ADS ANN equation file is empty: {equation_path}")
    missing_outputs = [
        f"ann_out_{index}"
        for index in range(1, len(manifest["output_columns"]) + 1)
        if re.search(
            rf"(?<![A-Za-z0-9_])ann_out_{index}(?![A-Za-z0-9_])",
            equation,
        )
        is None
    ]
    if missing_outputs:
        raise ValueError(
            "Renamed ADS ANN equation does not define every manifest output: "
            + ", ".join(missing_outputs)
        )
    remaining_formals = sorted(set(re.findall(r"(?<![A-Za-z0-9_])_v[0-9]+", equation)))
    if remaining_formals:
        raise ValueError(
            "Renamed ADS ANN equation still contains unbound native inputs: "
            + ", ".join(remaining_formals)
        )
    template_path = PACKAGE_DIR / contract["netlist_template_file"]
    template = template_path.read_text()
    placeholder = "__ADS_ANN_EQUATION_FRAGMENT__"
    if template.count(placeholder) != 1:
        raise ValueError(
            f"ADS ANN netlist template must contain exactly one {placeholder} marker"
        )
    destination = PACKAGE_DIR / contract["netlist_file"]
    destination.write_text(template.replace(placeholder, equation))
    return destination.name
...
def main():
    ...
    mapped_sdd_netlist = write_sdd_netlist(manifest, mapped_sdd_equation)
    print(json.dumps({
        ...
        "mapped_sdd_netlist": mapped_sdd_netlist,
        "preflight": summary,
...
`Y=(identity-S)*inverse(identity+S)/{z0:g}`. Expose the resulting scalar elements
...
generated `{netlist['netlist_template_file']}` template with the exact ADS ANN
text formula and writes the complete `{netlist['netlist_file']}` RF subnetwork.
...
```text
IncludePath="./relative/path/to/package"
IncludeFiles[1]="{netlist['netlist_file']}"
UsePreprocessor=yes
```

Load that definition once, then instantiate `{netlist['module_name']}` as many
...
Those are evaluator artifacts rather than complete ADS subnetworks.

## Manual Schematic Fallback
"""
    else:
        reason = (
            str(netlist.get("reason"))
            if isinstance(netlist, dict)
            else "No automatic netlist contract was generated."
        )
        netlist_section = f"""## Automatic Netlist Status

The automatic NetlistInclude package is disabled for this export: {reason}

Use the manual wrapper below, or regenerate with a compatible final-fine ANN
target and `--ads-output-format all` (or `equation`).
...
def ads_ann_readme(
    ...
    if isinstance(netlist, dict) and bool(netlist.get("enabled")):
        ...
        netlist_parameter_rows = "\n".join(
            ...
            f"`{float(netlist['parameter_input_scales'][source]):.12g}` |"
            for source, identifier in zip(
                parameter_names,
                netlist["parameter_identifiers"],
            )
        )
        netlist_files = f"""
- `{netlist['netlist_template_file']}`: pre-generated N-port SDD wrapper template
- `{netlist['netlist_file']}`: completed NetlistInclude model written by `train_ads_ann.py`
- `ADS_ANN_INSTANCE_TEMPLATE.txt`: two copyable native ADS instance calls
""".rstrip()
        netlist_summary = f"""After native training, the script automatically
creates the ready-to-include `{netlist['netlist_file']}` subnetwork. It maps
...
package below the ADS workspace. On the top-level simulation schematic, place
one `NetlistInclude` and set:
...
The include loads the definition once. Instantiate `{netlist['module_name']}`
...
The generated relation is `model_value=instance_value/input_scale`. Pass the
physical ADS-side value. For a model trained on dimensionless micron counts,
export with `--parameter-input-scales 1um` and pass values such as `W=10um`.
For MDIF parameters already parsed to SI, keep the default scale `1.0` and pass
the normal ADS quantity such as `W=500um`.

The raw `{settings['output_prefix']}.equation` remains an evaluator fragment;
do not include it directly. The generated `.net` file is the complete ADS
subnetwork.

## Step 4: Optional Manual Schematic Fallback
"""
        netlist_artifact_row = (
            f"| `{netlist['netlist_file']}` | Complete native ADS SDD subnetwork | "
            "Point the top-level `NetlistInclude` at this file; instantiate the "
            f"`{netlist['module_name']}` definition separately. |"
        )
        netlist_completion = f", and the complete `{netlist['netlist_file']}` subnetwork"
    else:
        reason = (
            str(netlist.get("reason"))
            if isinstance(netlist, dict)
        ...
        netlist_files = ""
        netlist_summary = (
            "The automatic NetlistInclude package is disabled for this export: "
            + reason
        )
        netlist_usage = """## Step 3: Manual Schematic Wrapper Required

Regenerate with final fine S-parameter outputs and `--ads-output-format all`
or `equation` to enable the automatic NetlistInclude/SDD package.

## Step 4: Create the Manual ADS Wrapper Cell
"""
        netlist_artifact_row = ""
        netlist_completion = ""
...
`train_ads_ann.py` retrains the tabulated inputs and outputs with ADS ANN. It
does **not** import the NumPy weights from `model.npz`. {netlist_summary}

The local fit and the ADS ANN fit can therefore differ. ADS ANN uses a uniform
hidden-layer width, while the source model may not. S-parameter and frequency
weights are recorded in the manifest but are not applied by this training
script because the verified ADS ANN API does not expose those per-output and
...
1. Use ADS 2025 or later with the Python ANN API. Keysight's ADS 2025 release
   notes state that the ANN Python API requires a Harmonic Balance license.
2. Keep this entire package together. A practical location is
   `<workspace>/models/{settings["output_prefix"]}/`; it does not need to be in
...
requested{netlist_completion}. When verification data exists, it also writes
`ads_ann_verification_prediction.csv`.

Before opening a schematic:

1. Confirm every expected native output exists.
2. Open `ads_ann_manifest.json` and record `input_columns` and
   `output_columns` in their listed order. Order is part of the interface.
3. Compare `truth_*` and `pred_*` columns in
   `ads_ann_verification_prediction.csv`. If this comparison fails, tune or
   regenerate the ADS ANN; a schematic wrapper cannot repair the ANN fit.
4. Check the fitted parameter limits in `ads_ann_training.csv`. The wrapper
   should reject, clamp, or explicitly warn about values outside these limits.
...
1. Create a new cell in your normal ADS library, for example
   `{settings["output_prefix"]}_nport`.
2. Create a schematic and symbol with one electrical pin per modeled port. The
   port order must match `sparam_labels` in `ads_ann_manifest.json`.
3. Add one design parameter for each name in `parameter_names`. These become
   instance parameters, so multiple symbols can use different values.
4. From Component Search, place the Symbolically Defined Device (SDD) with the
...
The ANN evaluator token names in `.equation` are generated by ADS. Do not
assume `_v1` means electrical port 1. For the ANN equation, the first formal
input maps to `input_columns[0]`, the second to `input_columns[1]`, and so on.
The SDD has its own `_v1`, `_i1`, ... electrical variables. Rename the ANN
formal inputs before combining the evaluator with SDD equations so those two
namespaces cannot collide.

## Step 5: Map Inputs Exactly

The scalar ANN inputs are, in order:

{inputs}

Geometry values in the CSV are numeric SI values because MDIF engineering
suffixes were parsed before export. Recommended instance conventions:

- If the ADS instance parameter is entered as `W=500um`, map `W_model=W`; ADS
  evaluates the engineering value as `0.0005`, matching the CSV.
- If the user-facing parameter is deliberately unitless, such as `W_um=500`,
  map `W_model=W_um*1e-6`.
- Do not pass `500` directly to an ANN trained on `0.0005`.

Frequency columns have these exact meanings:

- `freq_hz`: simulator frequency in hertz.
- `freq_log10_hz`: `log10(max(freq_hz, 1.0))`.
- A `log-linear` source model lists both columns, and both must be passed in the
  manifest order.

For SP/AC, map the ADS analysis frequency to these expressions. Do not use a
design variable that merely happens to have the same nominal value as the
controller frequency.

For a native KBNN residual/prior-input export, additional `coarse_*_real` and
`coarse_*_imag` inputs may follow the frequency columns. They must come from
the same coarse model, at the same geometry and frequency, used to create the
training CSV. For the simplest reusable ADS cell, regenerate the KBNN package
with `--ads-ann-target fine`; then these outputs are final fine S-parameters.

## Step 6: Reconstruct S and Stamp the Ports

The ANN scalar outputs are, in order:

{outputs}

All real columns precede all imaginary columns. Reconstruct each complex value
as, for example:
...
```text
Sij_fine = Sij_coarse + delta_Sij
```

For reference impedance `Z0`, convert the completed matrix with:

```text
Y = (identity - S) * inverse(identity + S) / Z0
Iport = Y * Vport
```
...
reference used by the S-to-Y conversion, not a physical termination.

For a two-port, the explicit conversion is useful for debugging:
...
If `D` approaches zero, the S-to-Y representation is ill-conditioned. First
compare the ANN's S outputs directly with the verification data; do not hide a
bad evaluator or port mapping by adding arbitrary conductance.

## Step 7: Make It Reusable at Multiple Parameter Values

Create the ANN evaluator and SDD stamp inside the wrapper cell, not on the top
testbench schematic. Expose parameters on the wrapper symbol. Then instances
can use independent values, for example:

```text
XANN1 W=420um L=1.1mm
XANN2 W=610um L=1.4mm
```

Each instance evaluates the same ANN coefficients with its own parameter
values. Global top-level `VAR` expressions may feed those instance parameters,
but the ANN generated files themselves are loaded only once by the wrapper.

## Step 8: Validate in Stages

1. **Native ANN only:** check `ads_ann_verification_prediction.csv`.
2. **Output mapping:** at one verification geometry, plot every reconstructed
   `Sij` before the S-to-Y conversion. This catches input order, units, and
   real/imaginary swaps.
3. **Electrical wrapper:** run an isolated SP simulation at the same geometry
   and compare all S-parameters with the source MDIF.
4. **Domain:** repeat at corners and interior verification points, including
   the lowest and highest fitted RF frequencies.
5. **Structure:** check reciprocity and the largest singular value. ADS ANN
   retraining does not inherit the source model's structural enforcement.
6. **Hierarchy:** instantiate two copies with different parameters and confirm
   that their responses differ as expected.
7. **Circuit:** only after the isolated tests pass, place the cell in the full
   circuit or optimization.

## Common Failure Signatures

| Symptom | Check first |
| --- | --- |
| `keysight.ads.ann` cannot be imported | The script is running under the ADS-bundled Python from the intended release and the required license is available. |
| ANN CSV verification is wrong | ADS ANN architecture, optimizer, iteration count, and training tolerance. The problem precedes the schematic. |
| ANN CSV is right but schematic S is wrong | Manifest input order, SI parameter scaling, frequency transform, real/imaginary output order, and S-port ordering. |
| S11/S22 look plausible but S12/S21 are swapped | `sparam_labels` order and the real-then-imaginary `output_columns` order. |
...
- [Measurement-Based ANN Simulation Models for RF Power Amplifiers](https://www.keysight.com/us/en/assets/3124-1550/application-notes/Measurement-Based-Artificial-Neural-Network-Simulation-Models-for-RF-Power-Amplifiers.pdf)
...
The Keysight application note establishes that an ANN equation is a computed
response and that an SDD or FDD supplies the electrical stimulus/response
shell. The installed ADS ANN documentation remains the authority for the exact
syntax emitted by your ADS release.
{notes}
"""
...
def write_ads_ann_package(
    ...
    requested_module_name = str(
        settings.get("netlist_module_name")
        or f"{settings['output_prefix']}_sdd"
    )
    z0 = float(settings.get("z0", 50.0))
    raw_parameter_scales = settings.get("parameter_input_scales")
    parameter_scales = (
        {str(name): float(value) for name, value in raw_parameter_scales.items()}
        if isinstance(raw_parameter_scales, dict)
        else {str(name): 1.0 for name in parameter_names}
    )
    settings["netlist_module_name"] = requested_module_name
    settings["z0"] = z0
    settings["parameter_input_scales"] = parameter_scales
    equation_requested = ".equation" in settings["expected_output_suffixes"]
    ads_netlist: dict[str, object]
    netlist_template_text: str | None = None
    if not equation_requested:
        ads_netlist = {
            "enabled": False,
            "reason": (
                "The selected ADS ANN output format does not produce a .equation "
                "file. Use --ads-output-format all or equation for the automatic "
                "NetlistInclude/SDD package."
            ),
        }
    else:
        try:
            parameter_defaults = np.mean(
                np.asarray(x_train, dtype=float)[:, : len(parameter_names)],
                axis=0,
            )
            netlist_template_text, ads_netlist = ads_ann_netlist_template(
                input_columns=input_columns,
                output_columns=output_columns,
                parameter_names=parameter_names,
                sparam_labels=sparam_labels,
    ...
    if netlist_template_text is not None:
        (out_dir / str(ads_netlist["netlist_template_file"])).write_text(
...
def _ads_hb_instance_template(
    ...
    call_a = _ads_hb_instance_call(module_name, "X1", nports, parameter_ids, "A")
    call_b = _ads_hb_instance_call(module_name, "X2", nports, parameter_ids, "B")
    return f"""; {title} instance-call template -- documentation only
...
def build_training_export_commands(
    ...
    *,
    ...
    dc_path = Path(dc_mdif).resolve() if dc_mdif else None
    ...
    if template_path is None:
        for filename in (
            ...
            candidate = resolved_model_dir / filename
            if candidate.exists():
...
def write_training_markdown(
    ...
    if (path.parent / ADS_EXPORT_TEMPLATE_FILENAME).exists():
        artifacts.append(
            ("ADS sampled-export template", ADS_EXPORT_TEMPLATE_FILENAME)
        )
