...
does **not** import the NumPy weights from `model.npz`. It also does not create
a ready-to-place electrical N-port. The native result is an ANN function. A
wrapper cell must map its scalar inputs, reconstruct the complex S-matrix, and
stamp the corresponding port currents.
...
## Step 3: Choose the Native Artifact
...
start with an SDD wrapper for SP/AC validation. Use the repository's dedicated
`export-ads-hb` model for harmonic balance; that export already handles
per-spectral-frequency evaluation, negative-frequency conjugation, and the
separate DC branch.

## Step 4: Create the ADS Wrapper Cell
...
| HB differs from SP | A simple SP SDD wrapper is not the repository's HB implementation. Use `export-ads-hb`, which evaluates every spectral frequency and handles negative-frequency symmetry. |
