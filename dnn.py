...
def train_model(args: argparse.Namespace) -> tuple[DNN, list[MDIFBlock], list[str], list[str], list[dict[str, float]], list[dict[str, float]], dict[str, object]]:
    ...
    if collocation_geometry_count and not passivity_enforced:
        raise ValueError(
            "Passivity collocation requires active S-domain passivity enforcement; "
            "use --output-domain s and --passivity-mode enforce (or passive source "
            "data with auto mode)"
        )
    ...
    if collocation_base_blocks:
        ...
        collocation_x, _collocation_placeholder_y = make_feature_target_samples(
            collocation_blocks,
            parameter_names,
            labels,
            args.freq_transform,
    ...
    def collocation_response_columns(predicted_scaled: np.ndarray) -> np.ndarray:
        return y_scaler.inverse_transform(np.asarray(predicted_scaled, dtype=float))

    auxiliary_loss_gradient = None
    auxiliary_score = None
    checkpoint_constraint = None
    if collocation_x_scaled is not None:
        assert passivity_loss_gradient is not None

        def auxiliary_loss_gradient(
            predicted_scaled: np.ndarray,
            _sample_indices: np.ndarray,
        ) -> tuple[float, np.ndarray]:
            return passivity_loss_gradient(
                predicted_scaled,
        ...
        def auxiliary_score(
            ...
            return passivity_max_singular_values_from_columns(
                collocation_response_columns(predicted_scaled),
                labels,
            )
    ...
    metadata = {
        ...
        "passivity_assessment_scope": (
            ...
            if collocation_x_scaled is not None
            else "positive-frequency training blocks only"
        ),
        **dc_metadata,
