...
def command_export_ads_ann(args: argparse.Namespace) -> int:
    ...
    if args.include_dc:
        if args.model_dir:
            saved_model = KBNN.load(Path(args.model_dir))
            if saved_model.parameter_names != list(parameter_names):
                raise ValueError(
                    "The saved KBNN parameter order does not match the ADS ANN training data"
                )
            if saved_model.sparam_labels != list(labels):
                raise ValueError(
                    "The saved KBNN S-parameter order does not match the ADS ANN training data"
                )
...
def command_rerank_sweep(args: argparse.Namespace) -> int:
    def after_promote(sweep_dir: Path, best_model_dir: Path) -> None:
        packaged_coarse_dir = sweep_dir / COARSE_MODEL_DIRNAME
        if packaged_coarse_dir.is_dir():
    ...
    return run_rerank_sweep_command(
        ...
        model_prefix="kbnn",
        result_columns=KBNN_SWEEP_RESULT_COLUMNS,
        export_commands_func=kbnn_export_commands,
...
def build_arg_parser() -> argparse.ArgumentParser:
    ...
    add_rerank_sweep_arguments(rerank, model_label="KBNN")
